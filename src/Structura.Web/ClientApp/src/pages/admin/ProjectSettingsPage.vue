...
<template>
  <div class="flex flex-col gap-6">
    <SkeletonRows v-if="projectQuery.isPending.value" :rows="6" />
    ...
    <template v-else-if="projectQuery.data.value">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-lg font-semibold text-text" dir="auto">{{ projectQuery.data.value.name }}</h1>
          <BaseBadge :variant="projectQuery.data.value.status === 'Active' ? 'success' : 'neutral'">
            {{ projectQuery.data.value.status }}
          </BaseBadge>
        </div>
      </div>
      ...
      <section class="rounded-xl border border-border bg-surface p-5">
        ...
        <form class="flex max-w-lg flex-col gap-4" @submit.prevent="saveMutation.mutate()">
          <BaseInput
            ...
            label="Name"
            :error="nameError"
            :disabled="projectQuery.data.value.status === 'Archived'"
            required
          />
          <BaseInput
            v-model="description"
          ...
          <div>
            <BaseButton
              ...
              :disabled="projectQuery.data.value.status === 'Archived' || !name.trim()"
            >
              Save Changes
            </BaseButton>
          </div>
      ...
      <section class="rounded-xl border border-border bg-surface p-5">
        ...
        <form
          v-if="projectQuery.data.value.status === 'Active'"
      ...
      <section
        v-if="projectQuery.data.value.status === 'Active'"
        class="rounded-xl border border-danger/30 bg-surface p-5"
      >
    ...
    <ConfirmDialog
      ...
      :message="`&quot;${projectQuery.data.value?.name}&quot; will become read-only for everyone.`"
